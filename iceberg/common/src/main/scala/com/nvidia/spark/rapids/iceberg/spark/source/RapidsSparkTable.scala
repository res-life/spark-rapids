 ...
 *   - `split-size`           -> [[SparkReadOptions.SPLIT_SIZE]]
 *   - `split-lookback`       -> [[SparkReadOptions.LOOKBACK]]
 *   - `split-open-file-cost` -> [[SparkReadOptions.FILE_OPEN_COST]]
 *
...
object RapidsSparkTable {
  ...
  /** Mapping from session-conf suffix to the iceberg read option key. */
  ...
  private[spark] def mergeSessionOptions(
    ...
    val sparkOpt = SparkSession.getActiveSession.orElse(SparkSession.getDefaultSession)
    if (sparkOpt.isEmpty) {
      return options
    }
    val sparkConf = sparkOpt.get.conf
    val tableKey = (Seq(catalogName) ++ namespace.toSeq :+ tableName).mkString(".")
