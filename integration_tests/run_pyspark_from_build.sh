...
then
    ...
    then
        ...
        #  - 6 GiB of host memory. In testing with a limited number of tasks (4) we saw
        #    the amount of host memory not go above 5.5 GiB.
        #  - 1 CPU core
        ...
        CPU_CORES=`nproc`
        HOST_MEM_PARALLEL=$(awk '/MemAvailable/ {print int($2 / (6 * 1024 * 1024))}' /proc/meminfo)
