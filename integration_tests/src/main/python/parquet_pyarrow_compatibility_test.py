...
def get_pa_type(data_gen):
    """Map from data gen type to pyarrow data type"""
...
def gen_pyarrow_table(data_gen, length=3, seed=0):
    ...
    assert(isinstance(data_gen, list))
...
def assert_gpu_and_cpu_are_equal_collect1()
#
...
@pytest.mark.parametrize('reader_confs', reader_opt_confs)
@pytest.mark.parametrize('v1_enabled_list', ["", "parquet"])
def test_parquet_read_round_trip_read_by_pyarrow(
        spark_tmp_path,
        parquet_gens,
        read_func,
        reader_confs,
    ...
    gen_list = [('_c' + str(i), gen) for i, gen in enumerate(parquet_gens)]
    data_path = spark_tmp_path + '/PARQUET_DATA'
    with_gpu_session(lambda spark: gen_df(spark, gen_list).coalesce(1).write.parquet(data_path))

    # 2. verify on pyarrow and GPU
    all_confs = copy_and_update(reader_confs, {
        'spark.sql.sources.useV1SourceList': v1_enabled_list,
        # set the int96 rebase mode values because its LEGACY in databricks which will preclude this op from running on GPU
        'spark.sql.legacy.parquet.int96RebaseModeInRead': 'CORRECTED',
