...
from iceberg import get_full_table_name, iceberg_unsupported_mark, _build_tblprops, \
    ...
    ICEBERG_V3_UNSUPPORTED_REASON, create_iceberg_unknown_table, \
    evolve_iceberg_unknown_columns, iceberg_v3_enabled_conf, insert_iceberg_unknown_rows
...
@pytest.mark.skipif(not supports_iceberg_v3, reason=ICEBERG_V3_UNSUPPORTED_REASON)
@ignore_order(local=True)
def test_iceberg_v3_unknown_type_cpu_write_gpu_read_and_evolution(
        spark_tmp_table_factory):
    table_name = get_full_table_name(spark_tmp_table_factory)

    def setup_table(spark):
        create_iceberg_unknown_table(spark, table_name)
        insert_iceberg_unknown_rows(spark, table_name)

    with_cpu_session(setup_table)

    def read_and_check_unknown_schema(spark):
        df = spark.table(table_name)
        assert df.schema["unknown_col"].dataType.simpleString() == "void"
        assert df.schema["nested"].dataType["unknown_col"].dataType.simpleString() == "void"
        return df

    assert_cpu_and_gpu_are_equal_collect_with_capture(
        read_and_check_unknown_schema,
        exist_classes="GpuBatchScanExec",
        conf=iceberg_v3_enabled_conf)

    with_cpu_session(lambda spark: evolve_iceberg_unknown_columns(spark, table_name))
    assert_cpu_and_gpu_are_equal_collect_with_capture(
