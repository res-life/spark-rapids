...
object GpuOrcTimezoneUtils {
   ...
   *
   * Uses the fused JNI kernel `GpuTimeZoneDB.convertOrcTimestampToSpark` for both same- and
   * cross-timezone reads. Even when the timezone rules match, the kernel must reconstruct the
   * writer-specific ORC 2015 base before deciding whether to apply the negative nanos borrow.
   *
   ...
   *
   * To match ORC Java, the JNI `convertOrcTimestampToSpark` kernel first applies the writer TZ
   * base offset and recomputes the negative nanos borrow, then applies any writer-to-reader TZ
   * delta and Spark's historical java.util.TimeZone-to-java.time rebase.
   */
    ...
    withResource(input) { _ =>
      if (containsOrcTimestamp(input)) {
        withResource(GpuTimeZoneDB.buildOrcTimezoneContext(writerTz, readerTz)) { tzCtx =>
          rebaseColumns(input, Some(tzCtx), writerUsedProlepticGregorian)
        }
      } else {
        rebaseColumns(input, None, writerUsedProlepticGregorian)
      }
    }
  }
  ...
  private def rebaseColumns(
    ...
    val newColumns = (0 until input.getNumberOfColumns).safeMap { colIdx =>
      ...
      if (dType == DType.TIMESTAMP_DAYS && !writerUsedProlepticGregorian) {
        ...
        convertOrcTimestamp(col, tzCtx.get)
      } else if (dType == DType.LIST || dType == DType.STRUCT) {
  ...
  private def convertOrcTimestamp(
      col: ColumnView,
      tzCtx: GpuTimeZoneDB.OrcTimezoneContext): ai.rapids.cudf.ColumnVector = {
